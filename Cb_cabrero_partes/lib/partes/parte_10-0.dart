...
PdfFont font = PdfStandardFont(PdfFontFamily.helvetica, 20, style: PdfFontStyle.bold);
    ...
    page1.graphics.drawString('10-0', font, bounds: Rect.fromLTWH(230, 90, page1.getClientSize().width, page1.getClientSize().height));
...
page2.graphics.drawString('Parte de emergencias', font ,bounds: Rect.fromLTWH(150, 70, page1.getClientSize().width,page1.getClientSize().height));
    page2.graphics.drawString('10-0', font, bounds: Rect.fromLTWH(230, 90, page1.getClientSize().width, page1.getClientSize().height));
...
page3.graphics.drawString('Parte de emergencias', font ,bounds: Rect.fromLTWH(150, 70, page1.getClientSize().width,page1.getClientSize().height));
    page3.graphics.drawString('10-0', font, bounds: Rect.fromLTWH(230, 90, page1.getClientSize().width, page1.getClientSize().height));
   ...
   if(compania != "Comandancia"){
    ...
    page1.graphics.drawString(
        'Tipo de incendio', PdfStandardFont(PdfFontFamily.helvetica, 15),
        bounds: Rect.fromLTWH(300, 640, page1.getClientSize().width,
            page1.getClientSize().height));

    page1.graphics.drawString(
        'Fase alcanzada', PdfStandardFont(PdfFontFamily.helvetica, 15),
        bounds: Rect.fromLTWH(
            5, 640, page1.getClientSize().width, page1.getClientSize().height));
    ...
    gridC2.columns[1].width = 100;
    _addGridRow(gridC2, 'T Construccion:', tconstruccion);
    _addGridRow(gridC2, 'Daños vivienda:', valorDanoVivienda);
    ...
    PdfGrid gridt = PdfGrid();
    gridt.columns.add(count: 1);
    gridt.columns[0].width = 180;

    

    _addGridRow13(gridt, opciontipo ?? '');

    _drawGrid(page1, gridt, const Rect.fromLTWH(300, 660, 0, 0));

    PdfGrid gridf = PdfGrid();
    gridf.columns.add(count: 1);
    gridf.columns[0].width = 180;

    

    _addGridRow13(gridf, opcionfase ?? '');

    _drawGrid(page1, gridf, const Rect.fromLTWH(0, 660, 0, 0));

    page2.graphics.drawString('5.- Otros inmuebles afectados',
        PdfStandardFont(PdfFontFamily.helvetica, 15),
        bounds: Rect.fromLTWH(
    ...
    for (var otrom in materialmayor) {
      _addGridRows2(
          gridmaterialm,
          otrom['Unidad'] ?? '',
          otrom['Maquinista'] ?? '',
...
Future<void> _saveAndLaunchFile(List<int> bytes, String unidad, String fecha) async {
  ...
  try {
    ...
    final fileName = '10-0_${unidad}_$fecha.pdf';
