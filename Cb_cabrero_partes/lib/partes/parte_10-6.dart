...
class Parte_10_6 {
  Future<void> createPDF_10_6(
      ...
      List<Map<String, String>> gasDatos,
...
PdfFont font = PdfStandardFont(PdfFontFamily.helvetica, 20, style: PdfFontStyle.bold);
    ...
    page1.graphics.drawString('10-6', font, bounds: Rect.fromLTWH(230, 90, page1.getClientSize().width, page1.getClientSize().height));
...
page2.graphics.drawString('Parte de emergencias', font ,bounds: Rect.fromLTWH(150, 70, page1.getClientSize().width,page1.getClientSize().height));
    page2.graphics.drawString('10-6', font, bounds: Rect.fromLTWH(230, 90, page1.getClientSize().width, page1.getClientSize().height));
...
page3.graphics.drawString('Parte de emergencias', font ,bounds: Rect.fromLTWH(150, 70, page1.getClientSize().width,page1.getClientSize().height));
    page3.graphics.drawString('10-6', font, bounds: Rect.fromLTWH(230, 90, page1.getClientSize().width, page1.getClientSize().height));
    ...
    page2.graphics.drawString('4.- Datos material peligroso',
        PdfStandardFont(PdfFontFamily.helvetica, 15),
        bounds: Rect.fromLTWH(
            0, 140, page2.getClientSize().width, page2.getClientSize().height));

      page2.graphics.drawString('Tipo de gas',
        PdfStandardFont(PdfFontFamily.helvetica, 13),
        bounds: Rect.fromLTWH(
            0, 160, page2.getClientSize().width, page2.getClientSize().height));

     page2.graphics.drawString('Contenedor',
        PdfStandardFont(PdfFontFamily.helvetica, 13),
        bounds: Rect.fromLTWH(
            130, 160, page2.getClientSize().width, page2.getClientSize().height));
      page2.graphics.drawString('Unidad ',
        PdfStandardFont(PdfFontFamily.helvetica, 13),
        bounds: Rect.fromLTWH(
            250, 160, page2.getClientSize().width, page2.getClientSize().height));
            page2.graphics.drawString('Cantidad',
        PdfStandardFont(PdfFontFamily.helvetica, 13),
        bounds: Rect.fromLTWH(
            330, 160, page2.getClientSize().width, page2.getClientSize().height));
      page2.graphics.drawString('Empresa',
        PdfStandardFont(PdfFontFamily.helvetica, 13),
        bounds: Rect.fromLTWH(
            400, 160, page2.getClientSize().width, page2.getClientSize().height));

    PdfGrid gridgas= PdfGrid();
    _createGrid(gridgas, 5);
     gridgas.columns[0].width = 125;
     gridgas.columns[1].width = 125;
     gridgas.columns[2].width = 75;
     gridgas.columns[3].width = 75;
     gridgas.columns[4].width = 100;

    
    for (var _gasDato in gasDatos) {
      _addGridRows22(
          gridgas,
          _gasDato['Gas'] ?? '',
          _gasDato['Contenedor'] ?? '',
          _gasDato['Unidad'] ?? '',
          _gasDato['Capacidad contenedor'] ?? '',
          _gasDato['Empresa'] ?? '');
    }

    
    _drawGrid(page2, gridgas, const Rect.fromLTWH(0, 180, 0, 0));
...
for (var i = 0; i < chunksAsistentes.length; i++) {
  ...
  void _addGridRows22(PdfGrid grid, String cell1Value, String cell2Value,
      String cell3Value, String cell4Value,String cell5Value) {
    PdfGridRow row = grid.rows.add();
    row.cells[0].value = cell1Value;
    row.cells[1].value = cell2Value;
...
Future<void> _saveAndLaunchFile(List<int> bytes, String unidad, String fecha) async {
  ...
  try {
    ...
    final fileName = '10-6_${unidad}_$fecha.pdf';
