import 'package:flutter_application_2/partes/parte_10-5.dart';
import 'package:flutter/material.dart';
import 'package:mask_text_input_formatter/mask_text_input_formatter.dart';

class Formulario10_5 extends StatefulWidget {
  const Formulario10_5({Key? key}) : super(key: key);

  @override
  _Formulario10_5 createState() => _Formulario10_5();
}

class _Formulario10_5 extends State<Formulario10_5> {
  ...
  final List<Map<String, String>> _matpelDatos = [];

  static final List<String> _matpelOpciones = [
    'Sólido combustible',
    'Sólido de reacción espontánea',
    'Sólido que reacciona con el agua',
    'Sólido venenoso',
    'Líquido combustible',
    'Líquido corrosivo',
    'Líquido infamable',
  ...
  final _controllers7 = {
    'Unidad(Kg o Lt)': TextEditingController(),
    'Cantidad': TextEditingController(),
    'Detalles': TextEditingController(),
  };
                    ...
                    child: Column(
                      children: [
                        Padding(
                          ...
                          child: DropdownButtonFormField<String>(
                            decoration: const InputDecoration(
                              labelText: 'Selecciona MATPEL',
                              border: OutlineInputBorder(),
                            ),
                            value: _seleccionMatpel,
                            items: _matpelOpciones
                        ...
                        ElevatedButton(
                          onPressed: () {
                            if (_controllers7.values.every((controller7) =>
                                ...
                                _seleccionMatpel != null) {
                              setState(() {
                                
                                _matpelDatos.add({
                                  for (var entry in _controllers7.entries)
                                    entry.key: entry.value.text,
                                  'MATPEL': _seleccionMatpel!,
                                });
                          ...
                          child: const Text('Agregar MATPEL'),
                        ),
                      ],
                    ),
                  ),
                  ...
                  if (_matpelDatos.isNotEmpty)
                    Padding(
                      ...
                      child: Container(
                        ...
                        child: Column(
                          children: [
                            const Text('Agregar MATPEL'),
                            IgnorePointer(
                              child: ListView.builder(
                                shrinkWrap: true,
                                physics: const NeverScrollableScrollPhysics(),
                                itemCount: _matpelDatos.length,
                                itemBuilder: (context, index) {
                                  var datosmatpels = _matpelDatos[index];
                                  return ListTile(
                                    title: Text(datosmatpels['MATPEL']!),
                                    subtitle: Column(
                                      crossAxisAlignment:
                                          CrossAxisAlignment.start,
                                      children: [
                                        for (var entry in datosmatpels.entries)
                                          if (entry.key != 'MATPEL')
                                            Text(
